...
- **File paths in the API are always relative to the project's `markdowns/`** — never absolute
...
### Orphans
Files in `projects/{name}/markdowns/` not referenced in `collection.yaml`. Shown at the bottom of the sidebar; can be dragged into the hierarchy.
...
**Orphan ordering:** `orphanSort: "recent" | "alpha" | "custom"` in Sidebar; drag-to-reorder auto-switches to `"custom"`. Resets on project switch.
...
- **Toggle behavior**: `handleUndo` replaces the top stack entry (current → saved) instead of popping — clicking ↩ again reverses the undo
